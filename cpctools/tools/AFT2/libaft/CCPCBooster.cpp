...
// e.g /dev/ttyS0, /dev/ttyUSB0, /dev/ports/usb0, COM27...

CCPCBooster::CCPCBooster(std::string comPort):
_COMPort(comPort),
...
void CCPCBooster::OpenPort()
{
  ...
  // YES, one equals sign; it's an assignment!
	if ((_COMPortHandle = RS232_OpenComport(_COMPort.c_str(), 115200, "8N1")) >= 0)
	{
...
// Non-blocking read of a byte. returns number of bytes read.
...
int CCPCBooster::ReadWaitWord(unsigned short &val)
{
...
{
  // I'm deprecating this; it will randomly eat 0-2 bytes and not tell you how many.
  return 0;
}
